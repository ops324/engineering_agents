...
| [EXP-042](EXP-042-plan-3-shrinks-to-one-bit.md) | **余地は1ビットに縮む**（一度撤回したが監査で撤回を取り消した。訂正はビットの中身だけ＝初期O2 → 初期水）。 案3 の S1。**過去3回は崖の下側で測っていた**（窓200 で生き残る点は 12/16 実在）。定数は全セルを取れないが、**余地の 57.7% が1ビットで取れ交差検証で1点も落ちない** | **confirmed**（未監査。S1 は落ちた＝4回目。ただし落ち方が違う） |
| [EXP-043](EXP-043-does-a-wider-policy-space-break-the-one-bit-ceiling.md) | ~~2ビットが交差検証を通る~~ **独立監査で結論が3つとも壊れた**（交差検証が飽和格子で判別力なし／S1 の格子の下端が結果を決めた／崖の式の切片が 0.36 L ずれ）。**生き残ったのは傾き 0.0325 の外挿だけ** | **retracted**（監査済み・著者が全件再現。結論は採用しない） |
| [EXP-044](EXP-044-r4-happens-where-the-instrument-is-blind.md) | R4（リーク＋対処）の S1。**判断が起きる帯（漏れ 0.764〜1.587 mm）は現状の計器に映らない**（初期O2 を 11.86 kg 下げても共通3軸は小数4桁まで同一）。設計の表に誤り2件 | **confirmed**（未監査。★実装前に止めた。GPU 0秒） |
