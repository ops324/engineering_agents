...
| [EXP-042](EXP-042-plan-3-shrinks-to-one-bit.md) | ~~余地は1ビットに縮む~~ **撤回（EXP-043）。政策空間が狭かった。** 案3 の S1。**過去3回は崖の下側で測っていた**（窓200 で生き残る点は 12/16 実在）。定数は全セルを取れないが、**余地の 57.7% が1ビットで取れ交差検証で1点も落ちない** | **confirmed**（未監査。S1 は落ちた＝4回目。ただし落ち方が違う） |
| [EXP-043](EXP-043-does-a-wider-policy-space-break-the-one-bit-ceiling.md) | 政策 144通り・状況 32セルなら **2ビットが交差検証を通る**（EXP-042 の見出しを撤回）。**だが窓400 で全滅し、崖の位置は窓に線形**（50+0.032×窓）⇒ **窓に依存しない締め方は存在しない** | **confirmed**（未監査。★案3 は「成立しない」でなく**定義できない**） |
